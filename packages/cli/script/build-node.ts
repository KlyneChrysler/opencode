...
import { mainConfig, parserWorkerConfig } from "../vite.node.config"
...
for (const target of targets) {
  ...
  const bundledWorker = path.join(dir, "dist-node", "parser.worker.mjs")
  const assetHash = await hashNodeAssets(
    assets.map((asset) =>
      asset.key === "@opentui/core/parser.worker.js" ? { ...asset, source: bundledWorker } : asset,
    ),
  )
