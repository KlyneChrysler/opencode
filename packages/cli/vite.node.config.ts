// NOTE: This file has a bunch of hacks and I'm going to fix this in OpenTUI to by
// adding support for primites that can be used to load assets from application
// provided dir, stable keys for assets, and publish parser.worker.js as a
// package. Leaving this here for first iteration but its in the pipeline.
...
const assetsModule = path.resolve(dir, "src/node/assets.node.ts")

function prependImport(code: string, names: string[]) {
  return `import { ${names.join(", ")} } from ${JSON.stringify(assetsModule)}\n${code}`
}

function opentuiNodePlugin(mode: "main" | "parser", target: NodeTarget): Plugin {
  const coreAssets = new Set<string>()
  let nativeRewrites = 0
  let workerRewrites = 0
  return {
    name: "opencode:opentui-node",
    enforce: "pre",
    resolveId(id) {
      if (id === target.opentuiNativePackage) return "\0opencode-opentui-native"
    },
    load(id) {
      if (id === "\0opencode-opentui-native") {
        return `export { default } from ${JSON.stringify(path.resolve(dir, "src/node/opentui-native.ts"))}`
      }
    },
    transform(code, id) {
      if (id.includes("node_modules/@opentui/core/index-")) {
        const assets = code.replace(
          /await resolveBundledFilePath\(\(\) => import\("(\.\/assets\/[^\"]+)", \{ with: \{ type: "file" \} \}\), "[^"]+", import\.meta\.url\)/g,
          (_match, specifier: string) => {
            coreAssets.add(`@opentui/core/${specifier.slice(2)}`)
            return `await resolveOpenTuiAsset(${JSON.stringify(specifier.slice(2))})`
          },
        )
        const native = assets.replace(/await import\("@opentui\/core-[^"]+"\)/g, () => {
          nativeRewrites++
          return `await import(${JSON.stringify(target.opentuiNativePackage)})`
        })
        return native === code ? null : prependImport(native, ["resolveOpenTuiAsset"])
      }
      if (id.endsWith(`node_modules/${opentuiParserWorkerAsset}`)) {
        const transformed = code.replace(
          ...
          () => {
            workerRewrites++
            return "await resolveWebTreeSitterWasm()"
          },
        )
        return transformed === code ? null : prependImport(transformed, ["resolveWebTreeSitterWasm"])
      }
    },
    buildEnd() {
      const missing = opentuiParserAssets.filter((asset) => !coreAssets.has(asset))
      const unexpected = Array.from(coreAssets).filter(
        (asset) => !opentuiParserAssets.includes(asset as (typeof opentuiParserAssets)[number]),
      )
      if (mode === "main" && (missing.length > 0 || unexpected.length > 0 || nativeRewrites === 0)) {
        this.error(
          `OpenTUI rewrite mismatch: missing=${missing.join(",") || "none"} unexpected=${unexpected.join(",") || "none"} native=${nativeRewrites}`,
        )
      }
      if (mode === "parser" && workerRewrites !== 1) {
        this.error(`Expected one OpenTUI parser worker rewrite, got ${workerRewrites}`)
      }
    },
  }
}
...
process.env.OTUI_TREE_SITTER_WORKER_PATH = __ocPath.join(__ocAssetRoot, ${JSON.stringify(opentuiParserWorkerAsset)})
...
export function mainConfig(input: NodeBuildInput): UserConfig {
  return defineConfig({
    root: dir,
    plugins: [
      opentuiNodePlugin("main", input.target),
      rawTextPlugin(),
...
export function parserWorkerConfig(input: NodeBuildInput): UserConfig {
  return defineConfig({
    root: dir,
    plugins: [opentuiNodePlugin("parser", input.target)],
    resolve,
    ssr: { noExternal: true },
    build: {
      ssr: `node_modules/${opentuiParserWorkerAsset}`,
      target: "node26",
      outDir: "dist-node",
      emptyOutDir: false,
      minify: false,
      rollupOptions: { output: output("parser.worker.mjs") },
    },
  })
}
